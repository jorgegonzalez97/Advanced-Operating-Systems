There are two python scripts:
	...
	- peer.py: It makes one peer node (client + server) start running
...
To run the client the following command is needed:
	...
	python3 peer.py <IP_ADDRESS> <IND_SERVER_PORT> <OWN_SERVER_PORT> <FOLDER_NAME> <LOG_PATH>
...
<FOLDER_NAME>: Name of the folder where the CLIENT stores its files (e.g. client_files_1) 
		IMPORTANT NOTE: The folder must exist

<LOG_PATH>: Path to store the log of the peer (e.g. logs/log_1.log)


Once the client is running, it will automatically register for P2P downloads and it will show the following message: 
...
Then, the client waits until a command is written:
	...
	-get_files_list: The client receives a list of the files that all the peer nodes that are registered host and their respective sizes.

	-query <FILE>: The client sends a download request of <FILE> to the indexing server. Then, it can keep typing and sending more commands, while listening for a queryhit to download the file as soon as it receives it.
